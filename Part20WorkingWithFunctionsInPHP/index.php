...
// Такая задача часто возникает при обработке данных из БД. В БД мы можем хранить данные в виде нулей и единиц, либо коротких слов, а пользователю хотелось бы раскрыть смысл этих нулей и единиц (DONE).

function numberToWord($number, $arr) {
    foreach ($arr as $key => $value) {
        if ($number == 1 and $key == 1) {
            return $value;
        } else if ($number == 0 and $key == 0) {
            return $value;
        }
    }
}

$arr = array('1' => 'Работает!', '0' => 'Пусто!');
